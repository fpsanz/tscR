...
Clustering is an unsupervised machine learning technique widely used in several disciplines
such as machine learning, bioinformatics, image analysis or pattern recognition.
Clustering gene expression -- as well as in other omic disciplines -- is useful for finding
gene sets with similar behaviour and could provide useful information for the understanding
...
Clustering gene expression can be performed based on genes, samples, or time variable.
In the last case, named clustering time series gene expression, it's possible identify
genes with similar dynamic response to certain conditions.
...
Despite the fact that there are several generic R packages for the analysis of clustering time series
-- _kmlShape_ , _dtwclust_, _clue_, _TSclust_ --, there is no a single algorithm
that satisfactorily solves all problems and each clustering problem requires a specific approach. 
There are other specific packages to clustering time series in gene expression which seek 
minimize the high sensitivity to noise of other generic algorithms --  _TSMixclust_, _ctsGE_, _MFuzz_ -- .
Although these methods attempt to solve the difficult task of minimizing noise effect, in general, genes with similar magnitude of expression end up in same clusters and that is not always what the researcher may be seeking.
...
Sometimes the researcher's greatest interest is in revealing genes with similar time pattern (similar trajectories), although their expression levels are very different. Therefore, genes with similar time evolution are searched regardless of the expression magnitude.
...
Let's suppose we subject an individual to a treatment and measure the expression of three (or ten thousand) genes at the time the treatment begins, at one week two weeks and three weeks (Fig. 1).
