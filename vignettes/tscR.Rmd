...
```{r warning=F, fig.cap="Figure 1", echo=FALSE, fig.align='center'}
a <- c(10, 15, 17, 25)
b <- c(5, 8, 6, 9 )
c <- c(-19, -14, -12, -4)
x <- c(1, 2, 3, 4)
...
data(tscR)
df <- tscR
knitr::kable(head(round(df,3)), caption = "Table 1", align = c('c', 'c', 'c'))
```
...
There are two possible arrangements of temporal data within SummarizedExperiment (Fig. 3):
...
assembled into a new matrix (Fig 3.B). With this, the argument _sample_ refers to the
